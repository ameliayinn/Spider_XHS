...
class XHS_Apis():
    ...
    def unlike_note(self, note_id: str, cookies_str: str, proxies: dict = None):
        """
            ...
            返回: 操作结果，data 中包含 like_count（取消后的赞数）
        """
        res_json = None
        try:
            api = "/api/sns/web/v1/note/dislike"
            data = {"note_oid": note_id}
            headers, cookies, data_encoded = generate_request_params(cookies_str, api, data, 'POST')
            response = requests.post(self.base_url + api, headers=headers, data=data_encoded.encode('utf-8') if isinstance(data_encoded, str) else data_encoded, cookies=cookies, proxies=proxies)
            res_json = response.json()
    ...
    def uncollect_note(self, note_id: str, cookies_str: str, proxies: dict = None):
        ...
        res_json = None
        try:
            api = "/api/sns/web/v1/note/uncollect"
    ...
    def delete_note(self, note_id: str, cookies_str: str, proxies: dict = None):
        """
            删除笔记接口（注意：小红书Web端可能限制此接口调用）
            :param note_id: 笔记ID
            :param cookies_str: cookies
            返回: 操作结果
        """
        res_json = None
        try:
            api = "/api/sns/web/v1/note/delete"
            data = {"note_id": note_id}
            headers, cookies, data_encoded = generate_request_params(cookies_str, api, data, 'POST')
            response = requests.post(self.base_url + api, headers=headers, data=data_encoded.encode('utf-8') if isinstance(data_encoded, str) else data_encoded, cookies=cookies, proxies=proxies)
            if response.status_code != 200:
                return False, f"HTTP状态码: {response.status_code} - 可能接口已被Web端屏蔽", None
            res_json = response.json()
            success, msg = res_json.get("success", False), res_json.get("msg", "")
        except Exception as e:
            success = False
    ...
    def delete_note_comment(self, note_id: str, comment_id: str, cookies_str: str, proxies: dict = None):
        """
            删除评论接口
            :param note_id: 笔记ID
        ...
        res_json = None
        try:
            api = "/api/sns/web/v1/comment/delete"
